...
trucks_data = [("蓝翔挖掘机", 3, 3, 9), ("顺丰快递车", 2, 12, 6)]
network_data = []
...
def apply_algorithm():
    ...
    for t in trucks_data:
        for i in range(int(t[1])):
            trucks.append(Truck(t[0], float(t[2]), float(t[3])))
    ...
    demands = [0, 3, 3.4, 2.3, 2, 3, 2.5, 1.2, 4]
    network = Network(dists, names, demands, trucks)
    network.coverage_allocate()
    network.path_generate()
    for t in network.trucks:
        print(t)
    return "success"
